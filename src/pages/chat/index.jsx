...
function renderMsgBlock(message) {
  ...
  const html = enum_msg.map((key) => (
    <li
      className={
        message[key].user === userId ? styles["right"] : styles["left"]
      }
    >
      <div className={`${styles["msg"]} ${styles["msg-in"]}`}>
        <div className={styles["msg-img"]}>
          <div className={styles["contact-img"]}>
            <a href="#">
              {message[key].user === userId ? (
                <img
                  src="https://nyulangone.org/images/doctors/c/cohen/1831196526/jeffrey-m-cohen-square.jpg"
                  alt=""
                />
              ) : (
                <img
        ...
        <div className={styles["conversation"]}>
          <div className={styles["chat-content"]}>
            <p className={styles["content"]}>{message[key].message}</p>
            <p className={styles["time"]}>
              {new Date(message[key].created_at).getDate() +
                "/" +
                new Date(message[key].created_at).getMonth() +
                " - " +
                new Date(message[key].created_at).getHours() +
                ":" +
                new Date(message[key].created_at).getMinutes()}{" "}
              <span>○</span>
            </p>
          </div>
          {message[key].user === userId ? (
            <div
              style={{ color: "#2ecc71", textAlign: "right" }}
              className={styles["chat-name"]}
            >
              Doctor Chung
            </div>
          ) : (
            <div className={styles["chat-name"]}>Patient Phong</div>
          )}
        </div>
      </div>
    </li>
...
function HandleSubmit(event) {
  ...
  axiosClient.post(`/message`, messageToSend).then((res) => {
    console.log(res);
    if (res.status === 200) {
...
function Chat() {
  ...
  axiosClient
    .get("/message/conversation/6292468f24d7122d8e99035b")
    .then((res) => {
      setMsg(res.data);
    });
  const mainHTML = (
      ...
      <div className={styles["msg-container"]}>
        <div className={styles["chat-panel"]}>
          <div id="msgBlock" className={styles["messages"]}>
            <ul>{renderMsgBlock(msg)}</ul>
          </div>
        </div>
        <div className={styles["contact-panel"]}>
          <h2>Contacts</h2>
          <div className={styles["contact-search"]}></div>
              ...
              <li>
                <div className={styles["contact-item"]}>
                  ...
                  <div className={styles["contact-info"]}>
                    ...
                    <p className={styles["latest-msg"]}>test</p>
                  </div>
                  <div className={styles["contact-img"]}>
                    <a href="#">
                      <img
